...
- `build.sh --wheel` also drops `_native.so` into the `python/` source tree, so you can run
  **without installing**: `PYTHONPATH=python python example/9_gravity_compensation.py`.
