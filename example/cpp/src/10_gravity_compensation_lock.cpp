...
int main(int argc, char** argv) {
  try {
    if (example::has_flag(argc, argv, "--help") || example::has_flag(argc, argv, "-h")) {
      std::cout << "Usage: ./10_gravity_compensation_lock --port /dev/ttyACM0 --rate 200 --use_gripper=true\n";
      return 0;
    }
    ...
    const float lock_kp = static_cast<float>(example::arg_double(argc, argv, "--lock-kp", 8.0));
    const float lock_kd = static_cast<float>(example::arg_double(argc, argv, "--lock-kd", 1.0));
