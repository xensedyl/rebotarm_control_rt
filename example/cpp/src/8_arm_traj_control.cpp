...
int main(int argc, char** argv) {
  try {
    if (example::has_flag(argc, argv, "--help") || example::has_flag(argc, argv, "-h")) {
      std::cout << "Usage: ./8_arm_traj_control --port /dev/ttyACM0 --rate 150\n";
      return 0;
    }
    ...
    std::cout << "Connected. Input x y z [duration], state, or q.\n";
    std::cout << "Backend: C++/Pinocchio IK, then joint-space interpolation.\n";

    while (true) {
      const auto line_opt = example::prompt("target > ");
      if (!line_opt) break;
      ...
      if (values.size() < 3) {
        std::cout << "need x y z [duration]\n";
        continue;
      }
      const double duration = values.size() >= 4 ? values[3] : 2.0;
